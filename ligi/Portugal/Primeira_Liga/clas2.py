...
# CONFIG
# ============================================================================
DEFAULT_DATA_CSV = r"merged_data_Tylko_Cechy_Rankingowe.csv"
SEED = 42
np.random.seed(SEED)
...
def run_rating_pipeline_v2(csv_path):
    ...
    if "result" in df_full.columns:
        df_full["result_raw"] = df_full["result"].astype(str).str.upper().str.strip()
        valid_results = {"W", "D", "L"}
        ...
        raise ValueError("❌ Brak kolumny docelowej: result lub home_pts/away_pts")
    ...
    print(f"\n{'─'*55}")
    print(f"  {'SPLIT':<18} {'SEZONY':<22} {'MECZÓW':>8}")
    print(f"{'─'*55}")
    print(f"  {'TRAIN':<18} {str(train_seasons):<22} {len(X_tr):>8,}")
    print(f"  {'CALIBRATION':<18} {str(calibration_season):<22} {len(X_cal):>8,}")
    print(f"  {'HOLDOUT':<18} {str(holdout_season):<22} {len(X_ho):>8,}")
    if X_future is not None:
        print(f"  {'FUTURE':<18} {'(brak wyniku)':<22} {len(X_future):>8,}")
    print(f"{'─'*55}")
    print(f"  {'RAZEM (z wynikiem)':<18} {'':<22} {len(X_tr)+len(X_cal)+len(X_ho):>8,}")
    print(f"{'─'*55}")

    # Rozkład klas w każdym splicie
    for split_name, y_split in [("TRAIN", y_tr), ("CALIBRATION", y_cal), ("HOLDOUT", y_ho)]:
        counts = {c: int((y_split == c).sum()) for c in [0, 1, 2]}
        total  = len(y_split)
        row = "  ".join(f"{CLASS_NAMES[c]}={counts[c]:,}({counts[c]/total*100:.1f}%)" for c in [0,1,2])
        print(f"  {split_name:<14}: {row}")
    ...
    # ------------------------------------------------------------------------
    # 11b. SKUTECZNOŚĆ CO 5% BINU — WIN / DRAW / LOSS
    # ------------------------------------------------------------------------
    print_subheader("11b. SKUTECZNOŚĆ CO 5 BIN (HOLDOUT, BLEND CALIBRATED)")

    bin_edges = np.arange(0.0, 1.05, 0.05)  # 0%, 5%, ..., 100%
    bin_labels = [f"{int(b*100):02d}-{int((b+0.05)*100):02d}%" for b in bin_edges[:-1]]

    for cls_idx, cls_label in [(2, "WIN (gospodarz)"), (0, "LOSS (gospodarz)"), (1, "DRAW")]:
        p_cls   = p_ho_blend_cal[:, cls_idx]
        y_true_bin = (y_ho == cls_idx).astype(int)
        bin_idx = np.digitize(p_cls, bin_edges, right=False) - 1
        bin_idx = np.clip(bin_idx, 0, len(bin_edges) - 2)

        header = f"  {'Bin':>9}  {'Meczów':>7}  {'Trafionych':>10}  {'Skut.rzecz':>11}  {'Skut.pred':>10}"
        print(f"\n  ── {cls_label} ──")
        print(header)
        print(f"  {'─'*55}")
        for b in range(len(bin_edges) - 1):
            ...
            if n == 0:
                continue
            hits       = y_true_bin[mask].sum()
            real_pct   = hits / n * 100
            pred_mean  = p_cls[mask].mean() * 100
            bar        = "█" * int(real_pct / 5)
            print(f"  {bin_labels[b]:>9}  {n:>7,}  {hits:>10,}  {real_pct:>10.1f}%  {pred_mean:>9.1f}%  {bar}")
