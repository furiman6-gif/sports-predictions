...
def process_file(file_path):
    ...
    output_file = f"{prefix_name}_Tylko_Cechy_Rankingowe.csv"
    ...
    df = df.copy(); df['target'] = df.apply(get_target, axis=1); df = df.dropna(subset=['target'])
    def get_result(row):
        h=pd.to_numeric(row[score_home_col],errors='coerce'); a=pd.to_numeric(row[score_away_col],errors='coerce')
        if pd.isna(h) or pd.isna(a): return None
        if h>a: return "W"
    ...
    os_models={"PL":PlackettLuce(mu=1500,sigma=500,beta=200,tau=5),"BTF":BradleyTerryFull(mu=1500,sigma=500,beta=200,tau=5),
        "BTP":BradleyTerryPart(mu=1500,sigma=500,beta=200,tau=5),"TMF":ThurstoneMostellerFull(mu=1500,sigma=500,beta=200,tau=5),
        "TMP":ThurstoneMostellerPart(mu=1500,sigma=500,beta=200,tau=5)}
    os_ratings={p:{} for p in os_models}
    wl_models={"WL":WengLin(mu=1500,sigma=500,beta=200),"WL_AGR":WengLin(mu=1500,sigma=500,beta=150,gamma=8),
        "WL_CON":WengLin(mu=1500,sigma=500,beta=300,gamma=3)}
    wl_ratings={p:{} for p in wl_models}
    pr=FootballPageRank(); massey=MasseyRating(); colley=ColleyRating()
    keener_models={"KEEN":KeenerRating(),"KEEN_LAP":KeenerRating(mode="laplace"),"KEEN_NL":KeenerRating(mode="nonlinear")}
    melo_models={"mELO_2D":MultiDimElo(k_dims=2),"mELO_4D":MultiDimElo(k_dims=4,c_lr=0.08),"mELO_8D":MultiDimElo(k_dims=8,c_lr=0.05)}
    bars_models={"BARS":BARSRating(),"BARS_FAST":BARSRating(k_base=48,k_min=16,k_max=80,surprise_window=10,streak_threshold=2),
        "BARS_SLOW":BARSRating(k_base=20,k_min=4,k_max=48,surprise_window=30,streak_threshold=5)}
    ed_models={"ED":EloDavidson(),"ED_AUTO":EloDavidson(auto_nu=True),"ED_HOME":EloDavidson(auto_nu=True,home_advantage=50)}
    siamese_models={"SIAM_8D":SiameseEmbeddingSystem(embed_dim=8,hidden_dim=32),
        "SIAM_16D":SiameseEmbeddingSystem(embed_dim=16,hidden_dim=64)}
    lm_models={"LM":LambdaMARTRanking(retrain_every=200,history_window=100,num_leaves=31,n_estimators=100),
        "LM_LONG":LambdaMARTRanking(retrain_every=200,history_window=200,num_leaves=15,n_estimators=150)}
    rn_models={"RN":RankNetSystem(hidden_dim=64,retrain_every=200,train_window=2000,epochs=50,lr=0.003),
        "RN_DEEP":RankNetSystem(hidden_dim=128,retrain_every=200,train_window=3000,epochs=80,lr=0.002)}
    lw_models = {
        "LNET": ListwiseRankingSystem(mode="listnet", hidden_dim=64, retrain_every=200, epochs=80, lr=0.003),
        "LMLE": ListwiseRankingSystem(mode="listmle", hidden_dim=64, retrain_every=200, epochs=80, lr=0.003),
    }
    rlhf_models = {
        "RLHF": RLHFSystem(hidden_dim=64, retrain_every=200),
        "RLHF_DEEP": RLHFSystem(hidden_dim=128, retrain_every=200, epochs=80),
    }
    ...
    print(f"  Wyliczanie ({total} meczow x {mc} modeli)...")
    for idx, row in df.iterrows():
        ...
        if mn%GRAPH_RECALC_EVERY==0 and mn>0:
        ...
        all_graph.append((ht,at,hg,ag,mn))
        ...
        for p,b in bars_models.items():
            ...
            b.update(ht,at,outcome,match_idx=mn)
        for p,ed in ed_models.items():
            ...
            ed.update(ht,at,outcome,is_home_a=ih)
        for prefix,siam in siamese_models.items():
            if siam.should_retrain():
            ...
            siam.add_match(ht,at,target)
        for prefix,lm in lm_models.items():
            if lm.should_retrain():
                print(f"   Retrenowanie {prefix} (mecz #{mn})...")
                lm._train()
            ...
            lm.add_match(ht,at,hg,ag,outcome)
        for prefix,rn in rn_models.items():
            if rn.should_retrain():
                print(f"   Retrenowanie {prefix} (mecz #{mn})...")
                rn._train()
            ...
            rn.add_match(ht,at,hg,ag,outcome)
        for prefix, lw in lw_models.items():
            if lw.should_retrain():
                print(f"   Retrenowanie {prefix} (mecz #{mn})...")
                lw._train()
            ...
            lw.add_match(ht, at, hg, ag, outcome)
        for prefix, rlhf in rlhf_models.items():
            ...
            rlhf.add_match(ht, at, hg, ag, outcome)
