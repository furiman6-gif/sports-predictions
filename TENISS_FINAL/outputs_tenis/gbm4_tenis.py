...
CSV_PATH  = str(_csv_new if _csv_new.exists() else (_csv5 if _csv5.exists() else _csv2))
...
def add_rolling_features(long):
    ...
    for metric in ["won", "sets_won", "games_won"]:
        shifted = long.groupby(["player", "Surface"])[metric].shift(1)
        for w in [5, 10]:
            rolled = shifted.groupby([long["player"], long["Surface"]]).rolling(w, min_periods=1).mean()
            long[f"{metric}_surf_avg{w}"] = rolled.reset_index(level=[0, 1], drop=True).values
...
def compute_elo(df, k=32, initial=1500, surface_specific=False):
    ratings = {}
    p1_elos, p2_elos = [], []
    ...
    for idx, row in df.iterrows():
        ...
        ratings[key1] = r1 + k * (s1 - e1)
        ratings[key2] = r2 + k * ((1 - s1) - (1 - e1))
...
def main():
    ...
    print("ELO / Glicko...")
    df_all = compute_elo(df_all, k=32, surface_specific=False)
    df_all = compute_elo(df_all, k=32, surface_specific=True)
    ...
    if roll_cols_present and "Surface" in train_df.columns:
        ...
        print(f"  Imputacja roll stats (rank_group×surface): {before:,} → {after:,} NaN w train")
    ...
    valid_pred = (valid_proba_cal >= 0.5).astype(int)
    test_pred  = (test_proba_cal  >= 0.5).astype(int)
    ...
    base_cols = [c for c in [DATE_COL, "P1", "P2", "Surface", "Round", "Tournament", TARGET_COL, "Season"] if c in test_df.columns]
    pred_df = test_df[base_cols].copy()
    pred_df["prob_P1_wins"] = test_proba_cal
    pred_df["prob_P2_wins"]     = 1 - test_proba_cal
    pred_df["pred"]    = ["P1" if p >= 0.5 else "P2" for p in test_proba_cal]
    ...
    if len(future_df) > 0 and len(X_future) > 0:
        ...
        fut_base = [c for c in [DATE_COL, "P1", "P2", "P1Rank", "P2Rank", "Surface", "Round", "Tournament"] if c in future_df.columns]
        fut_pred_df = future_df[fut_base].copy()
        fut_pred_df["prob_P1_wins"] = future_proba_cal
        fut_pred_df["prob_P2_wins"]     = 1 - future_proba_cal
        fut_pred_df["pred"] = ["P1" if p >= 0.5 else "P2" for p in future_proba_cal]
